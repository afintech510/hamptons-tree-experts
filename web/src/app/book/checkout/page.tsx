...
import { fetchQuote, submitBinOrder } from "@/lib/api";
...
function CheckoutForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await submitBinOrder(
      {
        ...
        <div className="mt-4 space-y-2 text-sm text-warm-gray-600">
          ...
          <div className="mt-3 border-t border-warm-gray-200 pt-3">
            <div className="flex items-baseline justify-between">
              <span className="font-semibold text-warm-gray-800">Total</span>
              <PriceDisplay
                cents={totalCents}
...
function CheckoutPageContent() {
  ...
  useEffect(() => {
    ...
    // Re-quote on mount only, not on every state change
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  if (!service || service.fulfillmentType !== "bin_immediate") {
    return (
